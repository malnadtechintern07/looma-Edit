...
class DatabaseManager:
    ...
    def update_profile(self, user_id_or_email, display_name=None):
        clean = (user_id_or_email or '').strip()
        if not clean or not display_name:
            return False
        ...
        if self.driver == 'mysql':
            ...
            with conn.cursor() as cur:
                ...
                """, (display_name.strip(), clean_email, clean))
                return cur.rowcount > 0
        else:
            conn = self._get_conn()
            cur = conn.cursor()
...
class RequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        # Check if email is registered (no credentials returned)
        if path == '/api/auth/check':
            ...
            self._send_json(200, {'exists': user is not None, 'email': email})
            return

        # Get Account By Email (returns credentials for local hash verification)
    ...
    def do_POST(self):
        ...
        # Save / Sync User Account (Directly from app registration or cloud sync)
        if path == '/api/auth/save-account':
            user_id = payload.get('id') or payload.get('userId') or ''
            email = (payload.get('email') or '').strip().lower()
            name = (payload.get('displayName') or payload.get('name') or 'ProCut Creator').strip()
            hash_val = payload.get('passwordHash') or payload.get('password_hash') or ''
            salt_val = payload.get('salt') or ''
            ...
            if not user_id:
                user_id = 'usr_' + hashlib.sha256(email.encode()).hexdigest()[:16]

            # If plain password passed without hash, generate hash & salt
            if not hash_val and payload.get('password'):
                salt_val = generate_salt()
                hash_val = hash_password(payload['password'], salt_val)
            ...
            user_record = {
                ...
                'salt': salt_val,
                'isPro': bool(payload.get('isPro', payload.get('is_pro', 1))),
                'createdAt': payload.get('createdAt') or datetime.utcnow().isoformat(),
                'lastLoginAt': payload.get('lastLoginAt') or datetime.utcnow().isoformat(),
            }

            DB.save_user(user_record)
            self._send_json(200, {'success': True, 'user': user_record})
            return
        ...
        if path == '/api/auth/update-profile':
            ...
            if not email and not user_id:
                self._send_json(400, {'error': 'Email or User ID is required'})
                return

            if display_name:
                DB.update_profile(email or user_id, display_name=display_name)

            updated = DB.get_user_by_email(email) if email else DB.get_user_by_id(user_id)
            if updated:
                res = {
                    'id': updated['id'],
                    'email': updated['email'],
                    'displayName': updated.get('display_name') or updated.get('displayName'),
                    'isPro': bool(updated.get('is_pro', 1)),
                    'lastLoginAt': updated.get('last_login_at', '')
                }
                self._send_json(200, res)
            else:
                self._send_json(200, {'success': True, 'displayName': display_name})
            return
        ...
        if path == '/api/projects/backup':
            ...
            if isinstance(project, dict):
                if not project.get('userId') and payload.get('userId'):
                    project['userId'] = payload['userId']
                if not project.get('userEmail') and payload.get('userEmail'):
                    project['userEmail'] = payload['userEmail']
